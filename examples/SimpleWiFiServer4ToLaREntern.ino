/*
 WiFi Web Server ToLaREntern Blink
 OMOMCで販売するToLaREnternをWEBから制御するためのプログラムとなっています。
...
{
    Serial.begin(115200);
    pinMode(LANTERN_PIN, OUTPUT);  // set the Lantern pin mode
        ...
        while (client.connected()) {            // loop while the client's connected
            if (client.available()) {             // if there's bytes to read from the client,
                ...
                if (currentLine.endsWith("GET /H")) {
                      lantern_on();               // GET /H turns the LED on
                }
                if (currentLine.endsWith("GET /L")) {
                    lantern_off();                // GET /L turns the LED off
                }
            }
        }
